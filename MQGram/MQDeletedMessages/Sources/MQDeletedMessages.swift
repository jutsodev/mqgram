...
// MARK: - Postbox-only helpers (MQDeletedMessages cannot import TelegramCore due to a circular dependency)

private func mqMediaTypeName(_ media: Media) -> String {
    let typeName = String(describing: type(of: media))
    switch typeName {
    case "TelegramMediaImage":
        return "image"
    case "TelegramMediaFile":
        return "file"
    case "TelegramMediaMap":
        return "location"
    case "TelegramMediaContact":
        return "contact"
    case "TelegramMediaPoll":
        return "poll"
    case "TelegramMediaDice":
        return "dice"
    case "TelegramMediaAction":
        return "action"
    case "TelegramMediaWebpage":
        return "webpage"
    case "TelegramMediaGame":
        return "game"
    case "TelegramMediaInvoice":
        return "invoice"
    case "TelegramMediaUnsupported":
        return "unsupported"
    default:
        return "other"
    }
...
public struct MQDeletedMessageInfo {
    ...
    public init(message: Message, deletedAt: Date = Date()) {
        ...
        self.authorName = mqPeerDisplayName(message.author)
        self.peerName = nil
        ...
        self.mediaTypes = message.media.map { mqMediaTypeName($0) }
    ...
    public static func searchDeletedMessages(
        ...
        return postbox.transaction { transaction -> [Message] in
            ...
            if let mediaType = filter.mediaType {
                ...
                // MQDeletedMessages is Postbox-only and cannot inspect TelegramMediaFile attributes
                // (video/voice/sticker/music/gif). Those subtypes all collapse to "file".
                let fileSubtypes: Set<String> = ["video", "voice", "sticker", "music", "gif", "video_message"]
                filtered = filtered.filter { msg in
                    for media in msg.media {
                        let typeName = mqMediaTypeName(media)
                        if typeName == normalized {
                            return true
                        }
                        if fileSubtypes.contains(normalized) && typeName == "file" {
                            return true
                        }
                    }
                    return false
                    ...
                    if let message = transaction.getMessage(messageId) {
                        ...
                        if message.media.isEmpty {
                            ...
                            for media in message.media {
                                mediaBreakdown[mqMediaTypeName(media), default: 0] += 1
                            }
                        }
        ...
        return postbox.transaction { transaction -> MQExportedData? in
            ...
            case .json:
                ...
                for msg in allMessages {
                    ...
                    var entry: [String: Any] = [
                        ...
                        "peer_name": mqPeerDisplayName(peer) ?? "",
                        "text": msg.text,
                    ...
                    if let author = msg.author {
                        ...
                        entry["author_name"] = mqPeerDisplayName(author) ?? ""
                    }
            ...
            case .csv:
                ...
                for msg in allMessages {
                    let attr = msg.mqDeletedAttribute
                    let peer = transaction.getPeer(msg.id.peerId)
                    let peerName = (mqPeerDisplayName(peer) ?? "").replacingOccurrences(of: ",", with: ";")
                    let authorName = (mqPeerDisplayName(msg.author) ?? "").replacingOccurrences(of: ",", with: ";")
            ...
            case .text:
                ...
                for msg in allMessages {
                    let attr = msg.mqDeletedAttribute
                    let peer = transaction.getPeer(msg.id.peerId)
                    let peerName = mqPeerDisplayName(peer) ?? "Unknown"
                    let authorName = mqPeerDisplayName(msg.author) ?? "Unknown"
                    let dateStr = dateFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(msg.timestamp)))
