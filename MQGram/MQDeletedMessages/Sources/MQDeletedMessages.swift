...
// MARK: - Peer Name Helper (avoids TelegramCore dependency)

private extension Peer {
    var mqDisplayTitle: String {
...
// MARK: - Media Type Helper (avoids TelegramCore dependency)

private extension Media {
    var mqMediaType: String? {
        ...
        if typeName.contains("TelegramMediaMap") { return "location" }
        if typeName.contains("TelegramMediaContact") { return "contact" }
        if typeName.contains("TelegramMediaPoll") { return "poll" }
        if typeName.contains("TelegramMediaDice") { return "dice" }
        if typeName.contains("TelegramMediaAction") { return "action" }
        return nil
    }
}
...
public struct MQDeletedMessageInfo {
    ...
    public init(message: Message, deletedAt: Date = Date()) {
        ...
        self.authorName = message.author?.mqDisplayTitle
        self.peerName = nil
        ...
        self.mediaTypes = message.media.compactMap { $0.mqMediaType }
        if let forwardInfo = message.forwardInfo {
            self.forwardAuthor = forwardInfo.author?.mqDisplayTitle
        } else {
            ...
            if let mediaType = filter.mediaType {
                filtered = filtered.filter { msg in
                    for media in msg.media {
                        if media.mqMediaType == mediaType { return true }
                    }
                    return false
                }
            }
                    ...
                    if let message = transaction.getMessage(messageId) {
                        ...
                        if message.media.isEmpty {
                            ...
                            for media in message.media {
                                let typeName = media.mqMediaType ?? "other"
        ...
        return postbox.transaction { transaction -> MQExportedData? in
            ...
            case .json:
                ...
                for msg in allMessages {
                    ...
                    if let author = msg.author {
                        ...
                        entry["author_name"] = author.mqDisplayTitle
                    }
            ...
            case .csv:
                ...
                for msg in allMessages {
                    let attr = msg.mqDeletedAttribute
                    let peer = transaction.getPeer(msg.id.peerId)
                    let peerName = (peer?.mqDisplayTitle ?? "").replacingOccurrences(of: ",", with: ";")
                    let authorName = (msg.author?.mqDisplayTitle ?? "").replacingOccurrences(of: ",", with: ";")
            ...
            case .text:
                ...
                for msg in allMessages {
                    let attr = msg.mqDeletedAttribute
                    let peer = transaction.getPeer(msg.id.peerId)
                    let peerName = peer?.mqDisplayTitle ?? "Unknown"
                    let authorName = msg.author?.mqDisplayTitle ?? "Unknown"
                    let dateStr = dateFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(msg.timestamp)))
